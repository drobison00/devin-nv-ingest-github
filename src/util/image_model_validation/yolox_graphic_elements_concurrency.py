...
from benchmarking.common_components import (  # noqa: E402
    ...
    yolox_graphic_grpc_infer_with_timing,
)
...
@click.option(
    "--output-file",
    default="yolox_graphic_elements_throughput_results.csv",
...
def optimize_throughput(
    ...
    """
    Tests the YOLOX graphic elements service to find the optimal throughput configuration.
    ...
    # Visualization-only is not implemented here to keep parity with page script usage
    if visualize_only:
...
def make_request_fn(
    ...
    if protocol == "http":
        ...
        def _fn(batch_size: int):
            return yolox_graphic_grpc_infer_with_timing(url, auth_token, prepped_input, batch_size, timeout)
