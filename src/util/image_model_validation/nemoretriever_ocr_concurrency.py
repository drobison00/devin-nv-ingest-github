...
@click.option(
    ...
    help="HTTP endpoint URL for the OCR model (e.g., http://host:8000/v1/infer).",
)
...
@click.option(
    "--output-file", default="nemoretriever_ocr_throughput_results.csv", help="CSV file to write results to.", type=str
)
...
def optimize_throughput(
    ...
    """
    Tests the NemoRetriever OCR HTTP service to find the optimal throughput configuration.

    - Sends batched HTTP requests with the payload format: {"input": [{"type":"image_url","url":"data:..."}, ...]}
    - Finds max stable batch size for each concurrency level over a stability duration.
    - Logs timing, throughput, status codes, and stability to CSV.
    """
    global running
    ...
    click.echo("\n=== Finding optimal throughput configuration (NemoRetriever OCR HTTP) ===")
    ...
    while current_concurrency <= max_concurrency and running:
        ...
        while current_batch <= max_batch and running and not batch_found:
            ...
            request_fn = make_request_fn(url, headers, timeout, data_url)
...
def make_request_fn(
    ...
    def _fn(batch_size: int):
        payload = build_http_payload_from_data_url(data_url, batch_size)
